...
function Get-UpdateEndpointUrl
{
  param(
    [Parameter(Mandatory = $true)]
    [string]$BaseUrl
  ...
  if ($trimmed.EndsWith('/api/update', [System.StringComparison]::OrdinalIgnoreCase))
  {
    return $trimmed
  }

  return "$trimmed/api/update"
}
...
function Invoke-DdnsUpdate
{
  ...
  $expectedBody = "OK: updated $ExpectedRecordType $ExpectedFqdn to $canonicalIpAddress"
...
$endpointUrl = Get-UpdateEndpointUrl -BaseUrl $FunctionBaseUrl
...
Write-Host "Smoke test target : $fqdn"
...
Write-Host ""
Write-Host "Step 1/4: Updating A record..."
Invoke-DdnsUpdate -EndpointUrl $endpointUrl `
                  -Client $ClientName `
                  -Key $ClientKey `
                  -DnsZone $Zone `
                  -RecordName $Name `
...
Write-Host "Step 3/4: Updating AAAA record..."
Invoke-DdnsUpdate -EndpointUrl $endpointUrl `
                  -Client $ClientName `
                  -Key $ClientKey `
                  -DnsZone $Zone `
